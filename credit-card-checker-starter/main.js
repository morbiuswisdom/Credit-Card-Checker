...
const validateCred = (arr) => {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    if (i % 2 == 0 && i != arr.length - 1) {
      // Checks if the index of the current number is even and the number is not the last index
      if (arr[i] * 2 > 9) {
        // Checks if the number * 2 is greater than 9
        reversedArr.push(arr[i] * 2 - 9);
      } else {
  ...
  const sum = reversedArr.reduce((a, b) => a + b, 0);
  return sum % 10 === 0;
};
